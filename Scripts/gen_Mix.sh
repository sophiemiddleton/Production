...
usage() { echo "Usage: $0
  ...
  [ --samopt (opt) Options to samListLocation default. DEPRECATED with jitfcl "-f --schema=root" ]
  ...
  [ --dsteps (opt)  If specified, use this list of input dts.*.art files instead of the one generated by SAM ]
    e.g.  Production/Scripts/gen_Mix.sh --primary CeEndpoint --campaign MDC2020 --pver t --mver p --over v --pbeam Mix1BB --dbpurpose perfect --dbversion v1_0"
}
...
cmd="mu2ejobdef --dsconf=${OUTCONF} --dsowner=${OWNER} --description=${OUTDESC} --embed mix.fcl --setup ${SETUP}\
