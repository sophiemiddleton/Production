...
def run(options,args):
  ...
  dirname = options.dirname #sys.argv[1]
  outpath = options.outpath #sys.argv[2]
  ...
  norms = {
    ...
    "Cosmic": cosmic_normalization(max_livetime),
  ...
  #TODO - how to input list of files - use sam?
  for line in ffns: 
    signal = line.split()[0]
    print(signal)
    ...
    for i in range(bl.GetEntries()):
      if bl[i].GetName().split("__")[0] == "mu2e::GenEventCount_genCounter": # TODO use something else here
        bn = bl[i].GetName()
    gen_events = 0
  ...
  while True: ##TODO ?
    ...
    # TODO not "mcs" files anymore
    d = {}
    d["datasets"] = datasets
    d["outnameMC"] = os.path.join(outpath,"dts.mu2e.ensemble-MC.MDC2024i.%06d_%08d.art" % (run,subrun))
    d["outnameData"] = os.path.join(outpath,"dts.mu2e.ensemble-Data.MDC2024i.%06d_%08d.art" % (run,subrun))
    d["run"] = run
    d["subRun"] = subrun
...
if __name__ == "__main__":
    ...
    parser.add_option('-a','--dirname', dest='dirname', default = 'dirname',help='dirname', metavar='dirname')
    parser.add_option('-c','--outpath', dest='outpath', default = 'outpath',help='outpath', metavar='outpath')
